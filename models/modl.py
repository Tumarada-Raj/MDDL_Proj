...
class myAtA(nn.Module):
    """
    performs DC step by making the image compatible with CNNS
    """
...
class data_consistency(nn.Module):
    ...
    def forward(self, z_k, x0, csm, mask, pretrained):
        if pretrained:
           self.lam = nn.Parameter(torch.tensor(0.05), requires_grad=False) 
...
class MoDL(nn.Module):
    def __init__(self, n_layers, k_iters, pretrained):
        """
        ...
        if self.pretrained:
            self.saver = CheckpointSaver('workspace/base_modlptw,k=10/checkpoints')
    ...
    def forward(self, x0, csm, mask):
        ...
        if self.pretrained:
            for k in range(self.k_iters):
                #dw
                x_k = x0.clone()
                self.dw = self.saver.load_pt('pretrained/ptm', 'final'+str(k+1), self.dw)
                z_k = self.dw(x_k) # (2, nrow, ncol)
    
                #dc
                x_k = self.dc(z_k, x_k, csm, mask, self.pretrained) # (2, nrow, ncol)
            return x_k
        else: 
            x_k = x0.clone()
            for k in range(self.k_iters):
                ...
                z_k = self.dw(x_k) # (2, nrow, ncol)
                #dc
                x_k = self.dc(z_k, x0, csm, mask, self.pretrained) # (2, nrow, ncol)
            return x_k