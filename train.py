...
def setup(args):
    ...
    '''if restore_weights and k_iters>1:
        restore_path = configs['restore_path']
        start_epoch, model = saver.load_pt(restore_path, restore_weights, model, k_iters)
    else:
        start_epoch = 0'''
...
def main(args):
    ...
    for epoch in range(start_epoch, epochs):
        for phase in phases: #['train', 'val'] or ['train']
            ...
            #write log
            if epoch%100 == 0:
                logger.write('Total no of Params:{}'.format(count_parameters(model)))
                logger.write('Total no of Trainable Params:{}'.format(count_trainable_parameters(model)))
            ...
            #if not pretrained and args.write_lambda:
