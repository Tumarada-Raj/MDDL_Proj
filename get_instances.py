...
class CheckpointSaver:
    ...
    def load_pt(self, restore_path, prefix, model):
        checkpoint_path = [os.path.join(restore_path, f) for f in os.listdir(restore_path) if f.startswith(prefix)][0]
        if prefix in ['final1','final2','final3','final4','final5','final6','final7','final8','final9','final10']:
            model = self.load_wts(checkpoint_path, model)
        else:
            raise NotImplementedError
        return model
    ...
    def load_wts(self, restore_path, model):
        #print('loading weights from {}...'.format(restore_path))
        state_dict = torch.load(restore_path)
        state_dict_v2 = copy.deepcopy(state_dict)
        for key in state_dict:
            if 'dw' in key:
                #print("conv",key.split('.'))
                pre, post, l, m = key.split('.')
                state_dict_v2[post +'.'+l+'.'+m] = state_dict_v2.pop(key)
            if 'dc' in key:
                pre, post = key.split('.')
                del state_dict_v2[pre + '.' + post]
